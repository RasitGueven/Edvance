...
2. **Prompt unten kopieren**, in Canva einfügen, generieren
3. **Format:** "Landscape (3:2)" wählen — matched unseren AssetList-Viewport
4. **Export:** PNG, 1200×800 reicht (Asset wird auf max-h-72 skaliert)
5. **Upload:** im Admin-UI `/admin/lambacher-preview` → bei der jeweiligen Aufgabe → Asset-Editor → "Bild hinzufügen" → Placeholder vorher per 🗑 entfernen

## Gemeinsame Style-Vorgabe

- **Stil:** flat vector illustration / minimalist line art
- **Hintergrund:** clean white
- **Ton:** friendly educational textbook style
- **Text auf Bild:** vermeiden ("no readable text") — Canva-AI verhaut Beschriftungen oft
- **Farben:** je nach Aufgabe kontextuell passend (siehe unten). Edvance-Blau ist eine Option, nicht die Pflicht.

Wenn ein Ergebnis zu generisch wirkt: `flat vector illustration` durch `isometric vector illustration` oder `hand-drawn educational illustration` ersetzen. Wenn zu kindisch: `minimalist line art` statt `flat vector illustration`.
...
**Canva-Prompt:**
> A handful of metallic wing nuts scattered on a clean light surface, each wing nut has two flat wing-shaped handles painted solid black on one side and solid white on the other, after being tossed roughly half lie flat with the black wing facing up, half lie flat with the white wing facing up, and a few stand balanced upright on their threaded base, top-down view from above, flat vector illustration, brushed steel and chrome metallic tones, stark black-and-white contrast on the painted wings, light grey background, educational textbook style
...
**Canva-Prompt:**
> A scattered group of about fifteen metallic thumbtacks on a clean cork board surface, top-down view from directly above, roughly two-thirds of the thumbtacks have landed flat with the round metal head facing up and the sharp pin pointing straight down, the remaining one-third have landed on their side with the round head tilted and the sharp pin visibly sticking out to one side, flat vector illustration, polished brass and silver metallic tones for the tacks, warm natural cork beige background, friendly educational textbook style
...
> A clean horizontal number line drawn across a white background showing integers from negative four to positive eight, a small marker labeled with the letter 'x' placed on the position three, a thin curved arrow pointing leftward from the x marker to position two suggesting predecessor, and another curved arrow pointing to position six suggesting doubling, flat minimalist vector illustration, dark charcoal line art on white, chalk-like texture, single-letter labels only

**Fallback (falls Zahlenstrahl zu mathy wirkt):**
> A friendly cartoon teenager sitting at a wooden desk with an open exercise book, writing simple algebra in pencil, soft warm desk-lamp lighting, a close-up of the open notebook page showing a chalk-style x symbol, flat vector illustration, warm study-room tones (amber lamplight, cream paper, dark wood desk), white background, educational textbook style, no readable text
...
- **Edvance-Farben optional:** Die Markenfarben (`#1B2A3E`, `#2D6A9F`, `#98C0D8`) passen gut für UI-Screenshots oder abstrakte Diagramme — aber für physische Objekte (Würfel, Münzen, Legosteine) sind die Originalfarben authentischer. Nur wenn ein Bild „zu bunt" wirkt oder nicht zur Seite passt: in Canva Color-Swap zu Edvance-Tönen. Mit Brand Kit (Canva Pro) `#1B2A3E` und `#2D6A9F` als Marken-Farben hinterlegen für schnelles Nachglätten.
- **Edit nach Generation:** in Canva selbst Sättigung/Kontrast anpassen falls Farben zu knallig wirken
