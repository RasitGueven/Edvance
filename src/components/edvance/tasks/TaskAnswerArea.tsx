...
export function TaskAnswerArea({
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-3">
      ...
      {hintShown && task.hint && (
        <div className="rounded-xl px-4 py-3 text-sm bg-[color-mix(in_srgb,var(--color-gold-warning)_10%,transparent)] border-2 border-[color-mix(in_srgb,var(--color-gold-warning)_30%,transparent)] text-[var(--color-gold-warning)]">
          💡 {task.hint}
        </div>
      )}
    </form>
