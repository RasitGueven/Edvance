// SetupScreen – session configuration screen before a diagnosis/screening starts.
...
const NO_TASKS_MSG = 'Keine diagnostischen Aufgaben in der Datenbank. Bitte Diagnostik-Content seeden.'
...
export function SetupScreen({
  ...
  const start = async (): Promise<void> => {
    setLoading(true)
    setError(null)
    const label = SUBJECT_OPTIONS.find(s => s.code === subject)?.label ?? 'Mathematik'
    ...
    if (active && active.generated_test) {
      ...
      const ratingBySnap = new Map((ratings ?? []).map(r => [r.behavior_snapshot_id, r.rating]))
      const snapshots: BehaviorSnapshot[] = []
      const snapshotIds: (string | null)[] = []
      ;(snaps ?? []).forEach((s, i) => {
        snapshots[i] = {
          ...(s as Omit<BehaviorSnapshot, 'coach_rating'>),
          coach_rating: (ratingBySnap.get(s.id) ?? null) as BehaviorSnapshot['coach_rating'],
        }
        snapshotIds[i] = s.id
      })
    ...
    const { tasks, test, warnings } = await buildRunTasks({ grade: student.class_level ?? grade, subject })
    ...
    startScreening({ studentName: name || 'Schüler', subject: label, tasks, screeningTestId: created.id })
  }
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-16">
      ...
      >
        ...
        <Button onClick={start} disabled={!name.trim() || loading} size="lg" className="mt-5 w-full">
