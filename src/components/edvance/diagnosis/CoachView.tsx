// CoachView – coach-facing rating screen during a diagnosis session.
...
import { Clock, Pencil, Lightbulb, AlertCircle, CheckCircle2, ArrowRight } from 'lucide-react'
import { ProgressBar } from './SessionHeader'
...
export function CoachView() {
  ...
  const rate = (rating: 1 | 2 | 3 | 4): void => {
    ...
    if (state.mode === 'db' && state.screeningTestId && snapId) {
      void createScreeningRating(snapId, state.screeningTestId, rating, user?.id ?? null)
    }
  }
        ...
        {currentSnapshot ? (
          <>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <BehaviorBadge icon={<Clock className="h-3 w-3" />} label={`Bedenkzeit ${(currentSnapshot.thinking_time_ms / 1000).toFixed(1)}s`} />
              <BehaviorBadge icon={<Clock className="h-3 w-3" />} label={`Dauer ${(currentSnapshot.task_duration_ms / 1000).toFixed(1)}s`} />
              <BehaviorBadge icon={<Pencil className="h-3 w-3" />} label={`${currentSnapshot.revision_count} Revisionen`} />
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {RATINGS.map(r => (
            <button
              ...
              style={{ background: r.color, borderBottom: `4px solid ${r.dark}` }}
            >
