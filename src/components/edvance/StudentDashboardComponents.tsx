...
const CLUSTER_TINTS = [
  ...
  { bg: 'color-mix(in srgb, var(--xp-gold) 14%, white)', fg: 'var(--color-accent-on)' },
]

// ── Cluster Grid ──────────────────────────────────────────────────────────────

export function ClusterGrid({ clusters }: { clusters: SkillCluster[] }) {
  return (
...
export function RowIcon({ type }: { type: ContentType }) {
