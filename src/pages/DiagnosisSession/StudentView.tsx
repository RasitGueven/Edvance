...
import { BehaviorBadge, ProgressBar } from './shared'
...
export function StudentView() {
  ...
  const handleSubmit = async () => {
    ...
    if (state.mode === 'db' && state.screeningTestId && user) {
      const { data } = await persistBehaviorSnapshot(task.id, user.id, snapshot, state.screeningTestId)
...
// Re-export BehaviorBadge used only here from shared to keep imports consistent.
export { BehaviorBadge }
