    ...
    <SessionShell maxWidth="md">
      <div className="flex flex-col gap-6">
        <button
          ...
          className="inline-flex min-h-[44px] items-center gap-1.5 self-start text-sm font-semibold text-white/60 hover:text-white"
        >
        ...
        <div>
          <p className="text-eyebrow text-white/55">{t('session.goal.eyebrow')}</p>
            ...
            {MOCK_SESSION_CHECKPOINTS.map((cp) => (
              <li key={cp.id} className="flex items-start gap-3">
                <CircleDot
                  ...
                  aria-hidden="true"
                />
                <span className="text-sm leading-relaxed text-white/90">{cp.label}</span>
              </li>
            ))}
          </ul>
        </EdvanceCard>
